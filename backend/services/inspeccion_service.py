...
class InspeccionService:
    ...
    def comparendos_to_rnmc(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        barrio = df["BARRIO_HECHOS"] if "BARRIO_HECHOS" in df.columns else df.get("LOCALIDAD", "")
        lugar = df["LUGAR"].astype(str) if "LUGAR" in df.columns else pd.Series("", index=df.index)
        return pd.DataFrame({
            "DTO": df.get("DTO", ""),
            # La exportación es de la unidad de Jamundí: cabecera, centros poblados y corregimientos.
            "MUNICIPIO": "JAMUNDI",
            "LOCALIDAD": barrio,
            "LUGAR": lugar,
            ...
            try:
                ...
                if not existing_act:
                    actuacion = InspeccionActuacion(
                        ...
                        id_registrador=str(row.get('ID_REGISTRA', '')) if source_format == "RNMC" else None,
                        funcionario=str(row.get('FUNCIONARIO', '')) if source_format == "RNMC" else None,
                        anotacion=str(row.get('ANOTACION', '')) if source_format == "RNMC" else None,
                        otros_medios_prueba=str(row.get('OTROS_MEDIOS_PRUEBA', '')) if source_format == "RNMC" else None,
                        fingerprint_hash=fp,
                ...
                # 4. Gestionar Finanzas (el formato de comparendos no trae valores: no se tocan)
                if source_format != "RNMC":
                    continue
