...
export const AlertsPanel = ({ alerts = [], updatedAt, onOpen, trayStatus }) => {
    const visibleAlerts = alerts
        ...
        .slice(0, 3);
    const statusBanner = trayStatus === 'ERROR'
    ...
    return (
        <article className="bg-white border border-slate-200 rounded-lg p-5 h-full">
            <div className="flex items-start justify-between gap-3 mb-4"><div><h3 className="font-black text-slate-900">Alertas semanales</h3><p className="text-xs text-slate-500">Bandeja central: evaluaciones persistidas y vigentes.</p></div><AlertTriangle size={20} className="text-amber-600" /></div>
            {statusBanner ? (
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-3"><p className="text-[10px] text-slate-500">Actualizado: {updatedAt ? new Date(updatedAt).toLocaleString('es-CO') : 'según última evaluación'} · SUPERSEDED solo en historial</p>{onOpen && <button onClick={onOpen} className="text-xs font-bold text-primary inline-flex items-center gap-1">Ver bandeja <ArrowRight size={14} /></button>}</div>
        </article>
    );
};
