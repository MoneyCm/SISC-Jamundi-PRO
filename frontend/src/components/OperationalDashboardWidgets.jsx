...
export const AlertsPanel = ({ alerts = [], updatedAt, onOpen, trayStatus, dataIncomplete = false }) => {
    ...
    return (
        <article className="bg-white border border-slate-200 rounded-lg p-5 h-full">
            <div className="flex items-start justify-between gap-3 mb-4"><div><h3 className="font-black text-slate-900">Señales semanales de la sábana</h3><p className="text-xs text-slate-500">Cambios semanales de la Policía que superan los umbrales · no son Alertas Tempranas.</p></div><AlertTriangle size={20} className="text-amber-600" /></div>
            {statusBanner ? (
                ...
                <div className="border border-emerald-100 bg-emerald-50 rounded-lg p-4 flex gap-3"><CheckCircle2 size={20} className="text-emerald-700 shrink-0" /><div><p className="text-sm font-bold text-emerald-900">Ninguna señal semanal supera los umbrales</p><p className="text-xs text-emerald-800 mt-1">{dataIncomplete ? 'Con salvedad: el periodo tiene semanas incompletas (ver aviso arriba). ' : ''}Las Alertas SISC de gestión y las señales del radar están en el Centro de análisis.</p></div></div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between gap-3"><p className="text-[10px] text-slate-500">Actualizado: {updatedAt ? new Date(updatedAt).toLocaleString('es-CO') : 'según última evaluación'} · Alertas reemplazadas en el historial</p>{onOpen && <button onClick={onOpen} className="text-xs font-bold text-primary inline-flex items-center gap-1">Ver en el Centro de análisis <ArrowRight size={14} /></button>}</div>
        </article>
    );
};
